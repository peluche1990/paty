var gatewayURL = 'http://paty.my.phpcloud.com/services';
var users = [ { 'name' : 'Static Joe', 'location' : 'San Francisco', 'state' : 'coding', 'phone' : '123' }];
 ...
 * be placed in <code>onGetCustomers</code>.
 */ 
...
/**
 * Handle response from GET /users
 * @param response
 * @returns
 */
function onGetCustomers(response) {
	...
	$.each(users, function(index, item) {
		newUsers += '<li data-theme="">' +
	            '<a href="#page2?empId='+index+'" data-transition="none">' +
	            item.name +
	        '</a>' +
	    '</li>';
	});
	$('#users li[role!=heading]').remove();
	$('#users').append(newUsers).listview('refresh');
}

function ajaxErrorHandler(xhr, ajaxOptions, thrownError) {
	jQuery.mobile.hidePageLoadingMsg();
	var _this = this;
	$(this).html('Ajax error. Trying static data!').show('slow', function() {
		onGetUsers({result : users});
		setTimeout(function(){
			$(_this).hide('slow');
		}, 500);
	...
	if (typeof data.toPage === 'string') {
		...
		if (r) {
			...
			if (user) {
				...
				$("#userstate").html('Is currently '+(user.state||'')+' in:');
				if (user.phone) {
					$("#usercall").attr('href', 'tel:'+user.phone).show().trigger('enhance');					
				} else {
					$("#usercall").hide();
				}
				var location = user.location;
				$("#location").attr("src", "https://maps.googleapis.com/maps/api/staticmap?center="+location+"&zoom=14&size=288x200&markers="+location+"&sensor=false");
			}
		}
	}
});
...
/**
 * Sends POST /addUser to API gateway.
 * 
 * This method is automatically generated. Don't modify it.
 * Application logic for handling the AJAX response should 
 * be placed in <code>onPostAddUser</code>.
 */ 
function postAddUser() {
	// This method is automatically generated. Don't modify it.
	jQuery.mobile.showPageLoadingMsg('Loading');
	$.ajax({
		url : gatewayURL + '/addUser',
		type: 'POST', 
		data : {
			'name' : $('#nameTxt').val(),
			'loc' : $('#locTxt').val(),
...
/**
 * Handle AJAX response for POST /addUser
 * 
 * @param response JSON object with response
 */
function onPostAddUser(response) {
